...
const querySelect = (item_key, property_key, comparetor, property_value) => {
    const query = datastore.createQuery(item_key).filter(new PropertyFilter(property_key, comparetor, property_value));
    return datastore.runQuery(query).then((items) => {
        return items[0].map(fromDatastore);
    });
}

module.exports = {STATE, postItem, deleteItem, querySelect}